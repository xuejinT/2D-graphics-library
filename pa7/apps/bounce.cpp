...
struct Shape {
    ...
    void setDeltaAngle(float da) { fDAngle = da; }
    ...
    void draw(GCanvas* canvas) {
        canvas->save();
        canvas->translate(fPos.fX, fPos.fY);
        canvas->rotate(fAngle);
        canvas->translate(-fPos.fX, -fPos.fY);

        fAngle += fDAngle;
        fAngle = fmodf(fAngle, 2*M_PI);
...
protected:
    ...
    GClick* onFindClickHandler(GPoint loc) override {
        ...
        int ang = 4 - fRand.nextS() % 8;
        fShapes[index].setDeltaAngle(ang / 400.0f);
